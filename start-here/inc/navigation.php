 ...
 * @since 1.0.0
 */
class Sh_Nav_Walker extends Walker_Nav_Menu {
...
function sh_menu_css_class($classes, $item) {
    ...
    return array_filter($classes, 'sh_is_element_empty');
}
add_filter('nav_menu_css_class', 'sh_menu_css_class', 10, 2);
add_filter('nav_menu_item_id', '__return_null');

function sh_menu_args($args = '') {
    $sh_menu_args['container'] = false;

    if (!$args['items_wrap']) {
        $sh_menu_args['items_wrap'] = '<ul class="%2$s">%3$s</ul>';
    }

    if (current_theme_supports('bootstrap-top-navbar') && !$args['depth']) {
        $sh_menu_args['depth'] = 2;
    }

    if (!$args['walker']) {
        $sh_menu_args['walker'] = new Sh_Nav_Walker();
    }

    return array_merge($args, $sh_menu_args);
}
add_filter('wp_nav_menu_args', 'sh_menu_args');
